...
name: canvas
description: |
  **The primary skill for terminal TUI canvases.** Covers spawning, controlling, and interacting with interactive terminal canvases via the canvas plugin tools.
...
**Start here when using terminal canvases.** This skill covers the overall workflow, canvas types, and IPC communication via the OpenCode canvas plugin tools.

## Plugin Tools

All canvas interaction happens through 5 plugin tools:

| Tool | Purpose |
|------|---------|
| `canvas_spawn` | Spawn a canvas in a tmux split pane |
| `canvas_update` | Send updated config to a running canvas via IPC |
| `canvas_selection` | Get the user's current selection from a canvas |
...
**Always use `canvas_spawn` for interactive scenarios** — this opens the canvas in a tmux split pane while keeping the conversation terminal available.

**Parameters:**
- `kind`: Canvas type (kanban, dashboard, pipeline, etc.)
- `config`: JSON configuration string for the canvas
- `scenario`: (optional) Interaction mode (for calendar/document/flight)
- `id`: (optional) Canvas instance ID for IPC — defaults to `<kind>-1`

### Example: Spawn a Kanban Board

```
canvas_spawn(
  kind: "kanban",
  config: '{"columns": [{"id": "todo", "title": "To Do", "wipLimit": 5, "cards": [{"id": "1", "title": "Task 1", "priority": "high", "labels": ["bug"]}]}, {"id": "doing", "title": "In Progress", "cards": []}, {"id": "done", "title": "Done", "cards": []}]}'
)
```
      ...
      "hunks": [
        {
          ...
          "lines": [
            ...
            {"type": "addition", "content": "const updated = false;"}
          ]
        }
      ]
    }
...
Interactive canvases communicate via Unix domain sockets. Use the plugin tools to interact:

- **`canvas_update`**: Push new configuration to a running canvas (re-renders instantly)
- **`canvas_selection`**: Retrieve the user's current selection (e.g. meeting time, flight, seat)
- **`canvas_content`**: Retrieve document content from a document canvas

### IPC Message Protocol (for reference)
...
- **tmux**: Canvas spawning requires a tmux session (plugin auto-starts one if needed)
